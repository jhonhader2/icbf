<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Teclados') }}</h2>
            <a href="{{ route('teclados.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 text-white rounded-md text-xs uppercase">{{ __('Nuevo') }}</a>
        </div>
    </x-slot>
    <div class="py-12">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($teclados as $t)
                            <tr><td class="px-6 py-4 text-sm">{{ $t->marca }}</td><td class="px-6 py-4 text-sm">{{ $t->modelo }}</td><td class="px-6 py-4 text-sm">{{ $t->serial }}</td><td class="px-6 py-4 text-right text-sm"><a href="{{ route('teclados.edit', $t) }}" class="text-indigo-600">Editar</a><form action="{{ route('teclados.destroy', $t) }}" method="POST" class="inline" onsubmit="return confirm('¿Eliminar?');">@csrf @method('DELETE')<button type="submit" class="text-red-600 ml-2">Eliminar</button></form></td></tr>
                        @empty
                            <tr><td colspan="4" class="px-6 py-4 text-center text-gray-500">No hay registros.</td></tr>
                        @endforelse
                    </tbody>
                </table>
                <div class="px-6 py-2">{{ $teclados->links() }}</div>
            </div>
        </div>
    </div>
</x-app-layout>
