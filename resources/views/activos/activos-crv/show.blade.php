<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Activo CRV') }}</h2>
            <a href="{{ route('activos-crv.index') }}" class="text-sm text-gray-600 hover:text-gray-900">{{ __('← Volver al listado') }}</a>
        </div>
    </x-slot>
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <dl class="grid gap-3 sm:grid-cols-2">
                    <div><dt class="text-sm font-medium text-gray-500">{{ __('Placa') }}</dt><dd class="mt-0.5 text-gray-900">{{ $activoCrv->placa ?? '—' }}</dd></div>
                    <div><dt class="text-sm font-medium text-gray-500">{{ __('Producto') }}</dt><dd class="mt-0.5 text-gray-900">{{ $activoCrv->producto?->nombre ?? $activoCrv->producto?->codigo ?? '—' }}</dd></div>
                    <div><dt class="text-sm font-medium text-gray-500">{{ __('Código producto') }}</dt><dd class="mt-0.5 text-gray-900">{{ $activoCrv->producto?->codigo ?? '—' }}</dd></div>
                    <div><dt class="text-sm font-medium text-gray-500">{{ __('Serie') }}</dt><dd class="mt-0.5 text-gray-900">{{ $activoCrv->serie ?? '—' }}</dd></div>
                    <div><dt class="text-sm font-medium text-gray-500">{{ __('Fecha adquisición') }}</dt><dd class="mt-0.5 text-gray-900">{{ $activoCrv->fecha_adquisicion?->format('d/m/Y') ?? '—' }}</dd></div>
                    <div><dt class="text-sm font-medium text-gray-500">{{ __('Costo histórico') }}</dt><dd class="mt-0.5 text-gray-900">{{ $activoCrv->costo_historico !== null ? number_format($activoCrv->costo_historico, 2, ',', '.') : '—' }}</dd></div>
                    <div><dt class="text-sm font-medium text-gray-500">{{ __('Depreciación') }}</dt><dd class="mt-0.5 text-gray-900">{{ $activoCrv->depreciacion !== null ? number_format($activoCrv->depreciacion, 2, ',', '.') : '—' }}</dd></div>
                    <div><dt class="text-sm font-medium text-gray-500">{{ __('Persona') }}</dt><dd class="mt-0.5 text-gray-900">@if($activoCrv->persona)<a href="{{ route('personas.show', $activoCrv->persona) }}" class="text-indigo-600 hover:text-indigo-800">{{ $activoCrv->persona->nombre ?? $activoCrv->persona->documento_identidad }}</a>@else — @endif</dd></div>
                    <div><dt class="text-sm font-medium text-gray-500">{{ __('Regional') }}</dt><dd class="mt-0.5 text-gray-900">{{ $activoCrv->regional?->nombre ?? '—' }}</dd></div>
                    <div><dt class="text-sm font-medium text-gray-500">{{ __('Bodega') }}</dt><dd class="mt-0.5 text-gray-900">{{ $activoCrv->bodega?->nombre ?? $activoCrv->bodega_codigo ?? '—' }}</dd></div>
                    @if($activoCrv->producto)
                        <div><dt class="text-sm font-medium text-gray-500">{{ __('Marca') }}</dt><dd class="mt-0.5 text-gray-900">{{ $activoCrv->producto->marca ?? '—' }}</dd></div>
                        <div><dt class="text-sm font-medium text-gray-500">{{ __('Modelo') }}</dt><dd class="mt-0.5 text-gray-900">{{ $activoCrv->producto->modelo ?? '—' }}</dd></div>
                    @endif
                </dl>
                @if($activoCrv->cpu)
                    @php($cpu = $activoCrv->cpu)
                    <div class="mt-6 pt-4 border-t border-gray-200">
                        <div class="flex justify-between items-center mb-3">
                            <h3 class="text-base font-medium text-gray-800">{{ __('CPU asociado') }}</h3>
                            <a href="{{ route('cpus.show', $cpu) }}" class="text-sm text-indigo-600 hover:text-indigo-800">{{ __('Ver ficha completa') }}</a>
                        </div>
                        <dl class="grid gap-3 sm:grid-cols-2">
                            ...
                            <div><dt class="text-sm font-medium text-gray-500">{{ __('Persona') }}</dt><dd class="mt-0.5 text-gray-900">@if($cpu->persona)<a href="{{ route('personas.show', $cpu->persona) }}" class="text-indigo-600 hover:text-indigo-800">{{ $cpu->persona->nombre ?? $cpu->persona->documento_identidad }}</a>@else — @endif</dd></div>
                        ...
                        @if($cpu->monitor || $cpu->teclado || $cpu->mouse)
                            <p class="mt-4 text-sm font-medium text-gray-700">{{ __('Periféricos asociados') }}</p>
                ...
                <div class="mt-6 flex gap-3">
                    <a href="{{ route('activos-crv.edit', $activoCrv) }}" class="px-4 py-2 bg-gray-800 text-white rounded-md hover:bg-gray-700">{{ __('Editar') }}</a>
                    <a href="{{ route('activos-crv.index') }}" class="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">{{ __('Volver') }}</a>
                </div>
            </div>
        </div>
    </div>
