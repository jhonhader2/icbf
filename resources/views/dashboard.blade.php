<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('personas.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800">{{ __('Ver personas') }}</a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <h3 class="text-base font-semibold text-gray-800 mb-4">{{ __('Total CPUs') }}</h3>
                    <div class="flex items-center gap-6">
                        <div class="flex-1" style="max-height: 200px;">
                            <canvas id="chartTotalCpus"></canvas>
                        </div>
                        <div class="text-center">
                            <p class="text-3xl font-semibold text-gray-900">{{ number_format($totalCpus) }}</p>
                            <a href="{{ route('cpus.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800 mt-1 inline-block">{{ __('Ver listado') }}</a>
                        </div>
                    </div>
                </div>
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
    ...
    <script>
        (function () {
            ...
            new Chart(document.getElementById('chartTotalCpus'), {
                type: 'doughnut',
                data: {
                    labels: ['{{ __("CPUs") }}'],
                    datasets: [{
